...
void *
logibulkcontroller_thread_send_upd_file(void *thread_data);
void *
...
static LogiBulkControllerPrivate *priv;
static void
...
G_DEFINE_TYPE_WITH_PRIVATE(LogiBulkController, logibulkcontroller, G_TYPE_OBJECT);
...
{
	g_debug("Initializing the bulk controller object");
...
logibulkcontroller_finalize(GObject *obj)
{
	g_debug("Destroying the controller object");
		...
		if (g_usb_interface_get_class(intf) == USB_INTERFACE_CLASS &&
			...
			if (SYNC_INTERFACE_SUBPROTOCOL_ID == g_usb_interface_get_subclass(intf)) {
				...
				g_debug("Interface = %u Address In = 0x%x Address Out = 0x%x",
					...
					priv->ctrl_device.sync_ep[IN_ENDPOINT],
					priv->ctrl_device.sync_ep[OUT_ENDPOINT]);
			} else if (UPD_INTERFACE_SUBPROTOCOL_ID ==
...
logibulkcontroller_claim_interface(gboolean do_claim)
{
	...
	if (do_claim) {
		ret = g_usb_device_claim_interface(priv->ctrl_device.device,
						   priv->ctrl_device.upd_interface_number,
		...
		ret =
		    g_usb_device_release_interface(priv->ctrl_device.device,
						   priv->ctrl_device.upd_interface_number,
						   G_USB_DEVICE_CLAIM_INTERFACE_BIND_KERNEL_DRIVER,
						   &error);
		if (!ret) {
...
logibulkcontroller_open_device(LogiBulkController *obj)
{
	...
	if (i == array->len) {
		g_warning("Did not find the device. Please connect the device %04x:%04x",
			  (guint)priv->ctrl_device.vid,
			  (guint)priv->ctrl_device.pid);
		return ERRORCODE_UNKNOWN_DEVICE;
	}
	g_debug("Found the device %04x:%04x",
		(guint)priv->ctrl_device.vid,
		(guint)priv->ctrl_device.pid);
...
static void
logibulkcontroller_transfer_cb_wrapper(GObject *source_object,
				       GAsyncResult *res,
				       gpointer user_data)
{
	gboolean ret;
...
static gint
logibulkcontroller_create_packet(UsbPacketSync *packet,
				 guint32 cmd,
				 guint32 length,
				 gint sequence_id,
				 const gchar *data)
{
	if (NULL == packet) {
		g_warning("Cannot create a packet with NULL address");
		return ERRORCODE_INVALID_PACKET;
	}
	packet->cmd = cmd;
...
logibulkcontroller_thread_read_sync_data(void *data)
{
	...
	while (priv->is_transfer_in_progress) {
		...
		g_usb_device_bulk_transfer_async(priv->ctrl_device.device,
						 priv->ctrl_device.sync_ep[IN_ENDPOINT],
						 (guint8 *)helper->buffer,
						 helper->buffer_len,
						 READ_ASYNC_TIME_OUT,
						 cancellable,
						 logibulkcontroller_transfer_cb_wrapper,
						 helper);
...
logibulkcontroller_send_data(guchar *write_buffer, guint32 length, gint interface_id)
{
	...
	if (BULK_INTERFACE_SYNC == interface_id) {
		...
		g_warning("Interface is invalid. Interface should be either UPD(%d) or Sync(%d)",
			  BULK_INTERFACE_UPD,
			  BULK_INTERFACE_SYNC);
		return ERRORCODE_INVALID_INTERFACE;
	}
	if ((ERRORCODE_NO_ERROR !=
	     (error_code = g_usb_device_bulk_transfer(priv->ctrl_device.device,
						      end_point,
						      (guchar *)write_buffer,
						      (gint)length,
						      &transferred,
		...
		error_code = (LIBUSB_ERROR_OTHER == error_code)
				 ? ERRORCODE_NO_ERROR
...
logibulkcontroller_check_ack(guint32 cmd, guint32 size)
{
	...
	if (ERRORCODE_NO_ERROR !=
	    (error_code = logibulkcontroller_read_data((guchar *)&priv->recv_pckt,
						       size,
						       BULK_INTERFACE_UPD,
						       (gint)READ_TIME_OUT))) {
...
static gint
logibulkcontroller_create_upd_packet_and_send(guint32 cmd,
					      guint32 length,
					      void *data,
					      gint *packet_len)
{
	...
	*packet_len = PACKET_HEADER_SIZE + length;
	...
	if (ERRORCODE_NO_ERROR !=
	    (error_code = logibulkcontroller_send_data((guchar *)&(priv->data_packet),
						       *packet_len,
						       BULK_INTERFACE_UPD))) {
...
static gint
logibulkcontroller_create_sync_packet_and_send(guint32 cmd,
					       guint32 length,
					       void *data,
					       gint sequence_id)
{
	...
	if (logibulkcontroller_create_packet(&send_data_pckt, cmd, length, sequence_id, data)) {
		g_warning("Sync packet creation failed.");
		return ERRORCODE_PACKET_CREATION_FAILED;
	}
	packet_length = length + SYNC_PACKET_HEADER_SIZE;
	if (ERRORCODE_NO_ERROR !=
	    (error_code = logibulkcontroller_send_data((guchar *)&send_data_pckt,
						       packet_length,
						       BULK_INTERFACE_SYNC))) {
		g_warning("Error in writing the data to the endpoint. ErrorCode: %d", error_code);
	}
...
logibulkcontroller_compute_hash(GBytes *data, guchar *base64hash)
{
	...
	md5buf[MD5_DIGEST_SIZE - 1] = NULL_CHARACTER;
	g_stpcpy((gchar *)base64hash,
		 (gchar *)g_base64_encode(md5buf, strlen((const gchar *)md5buf)));
...
logibulkcontroller_init_transfer(void)
{
	...
	/* Sending INIT */
	if (ERRORCODE_NO_ERROR !=
	    (error_code = logibulkcontroller_create_upd_packet_and_send(CMD_INIT,
									0,
									NULL,
									&(priv->packet_length)))) {
	...
	/* Receiving INIT ACK */
	if (ERRORCODE_NO_ERROR !=
	    (error_code = logibulkcontroller_check_ack(CMD_INIT, ACK_PKT_SIZE))) {
		g_warning("Error in reading acknowledgment for init transfer packet %d",
			  error_code);
		return error_code;
	}
...
logibulkcontroller_transfer_file_data(guchar *buf, guint size)
{
	...
	if (ERRORCODE_NO_ERROR !=
	    (error_code = logibulkcontroller_create_upd_packet_and_send(CMD_DATA_TRANSFER,
									size,
									buf,
									&priv->packet_length))) {
	...
	if (ERRORCODE_NO_ERROR !=
	    (error_code = logibulkcontroller_check_ack(CMD_DATA_TRANSFER, ACK_PKT_SIZE))) {
		g_warning("Error in reading acknowledgment for data transfer packet %d",
			  error_code);
	}
	return error_code;
}
...
logibulkcontroller_start_transfer(GBytes *file_data)
{
	...
	g_debug("Size of file to be transferred = %" G_GUINT64_FORMAT, trf_size);
	/* Transfer Sent*/
	if (ERRORCODE_NO_ERROR !=
	    (error_code = logibulkcontroller_create_upd_packet_and_send(CMD_START_TRANSFER,
									sizeof(trf_size),
									&trf_size,
									&priv->packet_length))) {
	...
	if (ERRORCODE_NO_ERROR !=
	    (error_code = logibulkcontroller_check_ack(CMD_START_TRANSFER, ACK_PKT_SIZE))) {
		g_warning("Error in reading acknowledgment for start transfer packet %d",
			  error_code);
		return error_code;
	}
	g_debug("[SUCCESS] : CMD_START_TRANSFER");
	priv->callback.bulk_transfer_cb(TRANSFER_STARTED,
					cur_per,
					BULK_INTERFACE_UPD,
					priv->user_data);
	...
	while (total_data_sent != length) {
		...
		if (ERRORCODE_NO_ERROR !=
		    (error_code = logibulkcontroller_transfer_file_data((buf + total_data_sent),
									packet_length))) {
			g_warning("Failed to send data packet : PACKET = %u", packet_count);
			priv->callback.bulk_transfer_cb(TRANSFER_FAILED,
							cur_per,
							BULK_INTERFACE_UPD,
							priv->user_data);
			return error_code;
		...
		if (cur_per != last_per) {
			priv->callback.bulk_transfer_cb(TRANSFER_INPROGRESS,
							cur_per,
							BULK_INTERFACE_UPD,
							priv->user_data);
		}
...
logibulkcontroller_finish_transfer(void)
{
	...
	if (ERRORCODE_NO_ERROR !=
	    (error_code = logibulkcontroller_create_upd_packet_and_send(CMD_END_TRANSFER,
	...
	if (ERRORCODE_NO_ERROR !=
	    (error_code = logibulkcontroller_check_ack(CMD_END_TRANSFER,
						       ACK_PKT_SIZE + ACK_PACKET_FOR_HASH_VALUE))) {
		g_warning("Error in reading acknowledgment for end transfer packet %d", error_code);
		return error_code;
	}
	g_debug("[SUCCESS]: CMD_END_TRANSFER");
	/*Send Uninit*/
	if (ERRORCODE_NO_ERROR !=
	    (error_code = logibulkcontroller_create_upd_packet_and_send(CMD_UNINIT,
									0,
									NULL,
									&priv->packet_length))) {
		g_warning("Error in writing finish transfer packet %d", error_code);
		return error_code;
	}
	/*Waitng for UninitAck*/
	if (ERRORCODE_NO_ERROR !=
	    (error_code = logibulkcontroller_check_ack(CMD_UNINIT, ACK_PKT_SIZE))) {
		g_warning("Error in reading acknowledgment for finish transfer packet %d",
			  error_code);
		return error_code;
	}
...
logibulkcontroller_thread_send_upd_file(void *data)
{
	...
	priv->callback.bulk_transfer_cb(TRANSFER_HASH_STARTED,
					0,
					BULK_INTERFACE_UPD,
					priv->user_data);
	if (ERRORCODE_NO_ERROR !=
	    (error_code = logibulkcontroller_compute_hash(priv->file_buffer, priv->base64hash))) {
		g_warning("Failed to compute hash for the given file data");
		priv->is_upd_transfer_in_progress = FALSE;
		err_str = g_strdup("Failed to compute hash for the given file data");
		priv->callback.bulk_error_cb(error_code,
					     BULK_INTERFACE_UPD,
					     err_str,
	...
	priv->callback.bulk_transfer_cb(TRANSFER_INIT_STARTED,
					0,
					BULK_INTERFACE_UPD,
					priv->user_data);
	...
	if (ERRORCODE_NO_ERROR !=
	    (error_code = logibulkcontroller_start_transfer(priv->file_buffer))) {
	...
	priv->is_upd_transfer_in_progress = FALSE;
	priv->callback.bulk_transfer_cb(TRANSFER_COMPLETED,
					100,
					BULK_INTERFACE_UPD,
					priv->user_data);
	priv->is_transfer_in_progress = TRUE;
	...
	while (priv->is_transfer_in_progress) {
		if (!g_queue_is_empty(priv->read_data_queue)) {
			...
			if (CMD_BUFFER_READ == data->cmd) {
				...
				priv->callback.bulk_read_cb_sync((const gchar *)data->payload,
								 data->length,
								 priv->user_data);
			...
			logibulkcontroller_create_sync_packet_and_send(CMD_ACK,
								       sizeof(cmd_data),
								       &cmd_data,
								       0);
		} else {
			g_mutex_lock(&priv->lock[READ_DATA_THREAD].mutex);
			LOCK(&priv->lock[READ_DATA_THREAD].cond,
			     &priv->lock[READ_DATA_THREAD].mutex);
...
logibulkcontroller_thread_send_data(void *thread_data)
{
	while (priv->is_transfer_in_progress) {
		if (!g_queue_is_empty(priv->send_data_queue)) {
			...
			if (!LOCK(&priv->lock[SEND_DATA_THREAD].cond,
				  &priv->lock[SEND_DATA_THREAD].mutex)) {
				gchar *err_str = (gchar *)"send data packet timed out";
				priv->callback.bulk_error_cb(ERRORCODE_BUFFER_TRANSFER_FAILED,
							     BULK_INTERFACE_SYNC,
							     err_str,
							     (guint32)strlen(err_str),
							     priv->user_data);
			...
			LOCK(&priv->lock[SEND_DATA_SYNC_THREAD].cond,
			     &priv->lock[SEND_DATA_SYNC_THREAD].mutex);
		...
		if (!g_queue_is_empty(priv->write_buffer_queue)) {
			...
			case CMD_ACK:
				if (CMD_BUFFER_WRITE == atoi((const gchar *)data->payload)) {
					logibulkcontroller_create_sync_packet_and_send(
					    CMD_UNINIT_BUFFER,
					    0,
					    NULL,
					    0);
				} else if (CMD_UNINIT_BUFFER ==
					   atoi((const gchar *)data->payload)) {
					priv->callback.send_data_cb_sync(ERRORCODE_NO_ERROR,
									 TRANSFER_SUCCESS,
									 priv->transaction_id,
									 priv->user_data);
			...
			LOCK(&priv->lock[WRITE_BUFFER_THREAD].cond,
			     &priv->lock[WRITE_BUFFER_THREAD].mutex);
...
{
	if (g_queue_is_empty(priv->write_buffer_queue) ||
	    !g_queue_is_empty(priv->send_data_queue) || !g_queue_is_empty(priv->read_data_queue)) {
		return TRUE;
	}
	return FALSE;
}
...
gint
logibulkcontroller_send_file_upd(LogiBulkController *obj,
				 GBytes *file_data,
				 gsize size,
				 gboolean start_update)
{
	...
	priv->send_file_upd_thread =
	    g_thread_new("SendUPDThread", &logibulkcontroller_thread_send_upd_file, NULL);
	return ERRORCODE_NO_ERROR;
}
...
logibulkcontroller_send_data_sync(LogiBulkController *obj, const gchar *data, guint32 size)
{
	...
	if (logibulkcontroller_create_packet(&send_data_pckt,
					     CMD_BUFFER_WRITE,
					     size,
					     transaction_id,
					     data)) {
		g_warning("Sync packet creation failed.");
		priv->ret.error_code = ERRORCODE_PACKET_CREATION_FAILED;
		return &priv->ret;
	}
...
logibulkcontroller_start_listening_sync(void)
{
	...
	priv->read_sync_data_thread =
	    g_thread_new("ReadSyncDataThread", &logibulkcontroller_thread_read_sync_data, NULL);
	priv->read_buffer_thread =
	    g_thread_new("ReadDataThread", &logibulkcontroller_thread_read_data, NULL);
	priv->send_data_thread =
	    g_thread_new("SendDataThread", &logibulkcontroller_thread_send_data, NULL);
	priv->write_buffer_thread =
	    g_thread_new("WriteBufferThread", &logibulkcontroller_thread_write_buffer, NULL);
}
...
logibulkcontroller_close_device(LogiBulkController *obj)
{
	...
	g_debug("Device closed successfully");
	return ERRORCODE_NO_ERROR;
}
