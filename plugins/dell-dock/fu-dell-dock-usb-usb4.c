...
static gboolean
fu_dell_dock_usb4_hub_get_mmio (FuDevice *device,
			        guint16 mbox_reg,
			        guchar *buf,
			        GError **error)
{
...
static gboolean
fu_dell_dock_usb4_hub_set_mmio (FuDevice *device, guint16 mbox_reg, guchar *buf,
			        GError **error)
{
	GUsbDevice *usb_device = fu_usb_device_get_dev (FU_USB_DEVICE (device));
 ...
 */
static gboolean
fu_dell_dock_usb4_mbox_data_read (FuDevice *device, guchar *data, guint8 length,
				  GError **error)
{
	guchar *ptr = data;
 ...
 */
static gboolean
fu_dell_dock_usb4_mbox_data_write (FuDevice *device, const guchar *data,
...
static gboolean
fu_dell_dock_usb4_hub_operation (FuDevice *device, guint16 opcode,
				 guchar *metadata, GError **error)
{
	struct mbox_regx *regx;
	gint max_tries = 100;
	guchar buf[4] = { 0x0 };
	...
	switch (opcode) {
		...
		case OP_DROM_READ:
			...
			if (!fu_dell_dock_usb4_hub_set_mmio (device,
							     MBOX_REG_METADATA,
							     metadata, error)) {
				g_prefix_error (error,
		...
		default:
			g_set_error (error, G_IO_ERROR, G_IO_ERROR_FAILED,
				     "invalid hub opcode: 0x%x", opcode);
			return FALSE;
	}
...
**/
static gboolean
fu_dell_dock_usb4_hub_nvm_read (FuDevice *device, guint8 *buf, guint32 length,
			        guint32 nvm_addr, GError **error)
{
	...
	while (length > 0) {
		...
		fu_common_write_uint32 (metadata,
					NVM_OFFSET_TO_METADATA(nvm_addr),
					G_LITTLE_ENDIAN);
		...
		if (!fu_dell_dock_usb4_hub_operation (device, OP_NVM_READ,
						      metadata, error)) {
			g_prefix_error (error, "hub NVM read error: ");
			return FALSE;
		}
		/* read the data from mbox data regs into our buffer */
		if (!fu_dell_dock_usb4_mbox_data_read (device, tmpbuf,
						       padded_len, error)) {
			g_prefix_error (error, "hub firmware mbox data read error: ");
			return FALSE;
		}
		if (!fu_memcpy_safe (buf, length, 0x0,
				     tmpbuf, sizeof(tmpbuf), unaligned_bytes,
				     nbytes, error))
			return FALSE;
				 ...
				 GError **error)
{
	...
	if (!fu_dell_dock_usb4_hub_operation (device, OP_NVM_SET_OFFSET,
					      metadata, error)) {
	...
	while (length > 0)  {
		...
		if (!fu_dell_dock_usb4_hub_operation (device, OP_NVM_WRITE,
						      NULL, error)) {
			    ...
			    FwupdInstallFlags flags, GError **error)
{
	...
	if (!fu_memcpy_safe (tmp_header, sizeof (guint32), 0x0,
			     fw_buf, fw_blob_size, 0x0, sizeof (guint32), error))
		return FALSE;
	...
	if (!fu_dell_dock_usb4_hub_nvm_read (device, nvm_buf, NVM_READ_LENGTH,
					     0, error)) {
	...
	if (((flags & FWUPD_INSTALL_FLAG_FORCE) == 0) &&
		...
		g_set_error (error,
			     ...
			     fw_vendor_id, fw_product_id,
			     nvm_vendor_id, nvm_product_id);
		return FALSE;
	}
...
fu_dell_dock_usb4_setup (FuDevice *device, GError **error)
{
	...
	nvm_version = g_strdup_printf ("%02x.%02x",
				       buf[NVM_VER_OFFSET_MAJOR],
				       buf[NVM_VER_OFFSET_MINOR]),
	nvm_vendor_id = g_strdup_printf ("%02x%02x",
					 buf[NVM_VID_OFFSET_MAJOR],
					 buf[NVM_VID_OFFSET_MINOR]);
	nvm_product_id = g_strdup_printf ("%02x%02x" ,
					  buf[NVM_PID_OFFSET_MAJOR],
					  buf[NVM_PID_OFFSET_MINOR]);
